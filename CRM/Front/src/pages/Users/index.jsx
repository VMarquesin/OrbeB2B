...
export default function GestaoUsuarios() {
  ...
  const [funcionarioPermissoes, setFuncionarioPermissoes] = useState(null);
  ...
  const handleAbrirPermissoes = async func => {
    setFuncionarioPermissoes(func);
    ...
    try {
      const resposta = await api.get(
        `/api/usuarios/${func.id}/permissoes`
      );
      ...
      showToast(
        'Erro ao carregar as permissões do colaborador.',
        'error'
      );
  ...
  const handleTogglePermissao = area => {
    setPermissoesSelecionadas(prev => {
      if (prev.includes(area)) {
        return prev.filter(permissao => permissao !== area);
      }
  ...
  const handleSalvarPermissoes = async () => {
    ...
    try {
      await api.put(
        `/api/usuarios/${funcionarioPermissoes.id}/permissoes`,
        {
          areas: permissoesSelecionadas
        }
      );
  ...
  const handleSalvarFuncionario = async e => {
    ...
    try {
      if (funcionarioEmEdicao) {
        ...
        await api.put(
          `/api/usuarios/${funcionarioEmEdicao.id}`,
          payloadPut
        );
  ...
  const handleConfirmarInativacao = async () => {
    ...
    try {
      await api.patch(
        `/api/usuarios/${funcionarioParaInativar.id}/inativar`
      );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white flex items-center gap-3">
            ...
            Controle de Acesso &amp; Colaboradores
          </h1>
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl overflow-hidden border border-slate-200">
                ...
                <>
                  <div className="mb-5">
                    <p className="text-sm font-semibold text-slate-600">
                      Selecione as áreas que este colaborador poderá acessar.
                    </p>

                    <p className="text-xs text-slate-400 mt-1">
                      Caso nenhuma permissão individual esteja definida,
                      o colaborador utilizará as permissões do perfil.
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {AREAS_PERMISSOES.map(area => {
                      const selecionada =
                        permissoesSelecionadas.includes(
                          area
                        );
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
              <button
                ...
                className="px-5 py-2.5 font-bold text-slate-500 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer"
              >
                Cancelar
              </button>
              ...
              <button
                ...
                className="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-bold rounded-xl transition-colors shadow-sm flex items-center gap-2 cursor-pointer"
              >
                {salvandoPermissoes ? (
                  <>
                    <Loader2
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden border border-slate-200">
            ...
            <div className="p-6 text-center space-y-4">
              ...
              <p className="text-slate-500 font-medium text-sm">
                ...
                Informe a senha temporária abaixo para o
                primeiro acesso:
              </p>
              ...
              <p className="text-xs text-slate-400">
                O colaborador deve alterar a senha no primeiro login.
              </p>
            </div>
