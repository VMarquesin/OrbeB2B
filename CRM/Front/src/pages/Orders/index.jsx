...
import {
  listarPedidosCRM,
  criarPedidoManualCRM,
  obterDetalhePedidoCRM,
  atualizarStatusPedidoLogistica
...
export default function GestaoOrcamentaria() {
  ...
  const [modoPF, setModoPF] = useState(false);
  ...
  const carregarPedidos = async () => {
    ...
    try {
      ...
      setErroPedidos(
        err.mensagemNormalizada ?? 'Erro ao carregar pedidos.'
      );
    } finally {
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      try {
        ...
        console.error(
          'Erro ao carregar opções do modal manual:',
          e
        );
      }
    };
  ...
  useEffect(() => {
    const dadosSalvos =
      localStorage.getItem('caseira_pcp_settings');
    ...
    if (dadosSalvos) {
      ...
      setConfigPcp({
        mesasFisicas:
          Number(config.mesasFisicas || 8),
        bandejasPorMesa:
          Number(config.bandejasPorMesa || 2),
        capacidadeBandeja:
          Number(config.capacidadeBandeja || 208)
      });
    }
  }, []);
  ...
  const formatarStatusLogistica = pedido => {
    ...
    const rawStatus =
      pedido.statusLogistica ??
      pedido.status_logistica ??
      pedido.status;
    ...
    if (
      rawStatus === 3 ||
      rawStatus === 4 ||
      strStatus.includes('conclu') ||
      strStatus.includes('entreg')
    ) {
      return {
        label: 'Concluído',
        color:
  ...
  const processedOrders = useMemo(() => {
    let filtered = pedidos.filter(o => {
      ...
      const codigo = (
        o.codigoPedidoFormatado ||
        o.codigo_pedido_formatado ||
        ''
      ).toLowerCase();
      ...
      const dataFormatada = dataOrigem
        ? new Date(dataOrigem).toLocaleDateString('pt-BR')
        : '';
      ...
      const statusInfo =
        formatarStatusLogistica(o);

      let matchesStatus = true;

      if (statusFilter === 'AGUARDANDO') {
        matchesStatus =
          statusInfo.value === 'aguardando';
      }

      if (statusFilter === 'PREPARACAO') {
        matchesStatus =
          statusInfo.value === 'preparacao';
      }

      if (statusFilter === 'CONCLUIDO') {
        matchesStatus =
          statusInfo.value === 'concluido';
      }

      const dataRegistro = dataOrigem
        ? new Date(dataOrigem).getTime()
        : 0;

      const limiteInicio = dataInicio
        ? new Date(
            dataInicio + 'T00:00:00'
          ).getTime()
        : 0;

      const limiteFim = dataFim
        ? new Date(
            dataFim + 'T23:59:59'
          ).getTime()
        : Infinity;
      ...
      const matchesTempo =
        dataRegistro >= limiteInicio &&
        dataRegistro <= limiteFim;
        ...
        const campoA =
          sortConfig.key === 'clienteNome'
            ...
            : sortConfig.key ===
              ...
              ? (
                  a.codigoPedidoFormatado ||
                  a.codigo_pedido_formatado ||
                  ''
                )
              : sortConfig.key ===
                  'data_criacao'
        ...
        const campoB =
          sortConfig.key === 'clienteNome'
            ...
            : sortConfig.key ===
                'codigo_pedido_formatado'
              ? (
                  b.codigoPedidoFormatado ||
                  b.codigo_pedido_formatado ||
                  ''
                )
              : sortConfig.key ===
                  'data_criacao'
  ...
  const handlePuxarUltimoPedido = async () => {
    ...
    if (pedidosValidos.length === 0) {
      alert(
        'Este cliente possui pedidos, mas nenhum pode ser repetido.'
      );
      return;
    }
    ...
    try {
      ...
      if (itensRepetidos.length === 0) {
        alert(
          'O último pedido encontrado não possui itens para repetir.'
        );
        return;
      }
      ...
      alert(
        `Último pedido (${
          ultimo.codigoPedidoFormatado ||
          ultimo.codigo_pedido_formatado
  ...
  const handleAddProduto = () => {
    ...
    const novoItem = {
      ...
      embalagem:
        produtoSelecionado.embalagem,
      quantidade:
        Number(qtdProduto),
      precoUnitario:
        Number(precoEditavel),
      eh_fabricacao_propria:
        produtoSelecionado.eh_fabricacao_propria,
      origem:
        produtoSelecionado.eh_fabricacao_propria
          ? 'proprio'
          : 'terceiro'
  ...
  const handleSalvarPedidoManual =
    async () => {
      if (
        ...
        setErroManual(
          'Selecione um cliente ou ative o modo Pessoa Física.'
        );
        return;
      }
      ...
      const payload = {
        ...
        itens:
          itensManuais.map(item => ({
            ...
            precoUnitario:
              Number(item.precoUnitario),
            ehFabricacaoPropriaSnapshot:
              item.eh_fabricacao_propria
          }))
      };

      try {
        await criarPedidoManualCRM(
          payload
        );

        const pedidosAtualizados =
          await listarPedidosCRM();
  ...
  const handleConfirmarAprovacao =
    async () => {
      ...
      try {
        await atualizarStatusPedidoLogistica(
        ...
        setErroTriagem(
          err.mensagemNormalizada ??
          'Erro ao aprovar o pedido. Tente novamente.'
        );
  ...
  const handleConfirmarConclusao =
    async () => {
      ...
      try {
        ...
        setErroTriagem(
          err.mensagemNormalizada ??
          'Erro ao concluir o pedido. Tente novamente.'
        );
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 print:hidden">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Esteira de pedidos Híbrida (App B2B e Lançamentos Manuais).
          </p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              ...
              {!loadingPedidos &&
                ...
                processedOrders.map(
                  pedido => {
                    const codigo =
                      pedido.codigoPedidoFormatado ||
                      pedido.codigo_pedido_formatado ||
                      '—';
                    ...
                    return (
                      ...
                      >
                        <td className="p-4">
                          <p className="font-bold text-slate-800 bg-slate-100 inline-block px-2 py-0.5 rounded text-xs">
                            {codigo}
                          </p>
                        ...
                        <td className="p-4">
                          ...
                          {resumo && (
                            <p className="text-xs font-medium text-slate-400 mt-0.5 truncate max-w-sm">
                              {resumo}
                            </p>
                          )}
                        </td>
          ...
          <div className="bg-white w-full max-w-3xl rounded-2xl shadow-xl overflow-hidden border border-slate-200 flex flex-col max-h-[90vh] print:max-h-none print:w-full print:h-full print:border-none print:shadow-none print:rounded-none">
            ...
            <div className="p-6 border-b border-slate-200 flex justify-between items-start bg-slate-50 print:bg-white print:border-b-2 print:border-slate-800 print:pb-6 print:mb-6">
              ...
              <div className="w-full">
                ...
                {/* CABEÇALHO DA EMPRESA - IMPRESSÃO */}
                <div className="hidden print:block">

                  <div className="flex justify-between items-start gap-6">

                    <div>
                      <h1 className="text-2xl font-black text-amber-500 uppercase tracking-tight">
                        A CASEIRA
                      </h1>
                      ...
                      <p className="text-sm font-black text-slate-800 mt-4">
                        {pedidoSelecionado?.razaoSocialEmpresa || '—'}
                      </p>

                      {pedidoSelecionado?.nomeFantasiaEmpresa &&
                        pedidoSelecionado.nomeFantasiaEmpresa !== pedidoSelecionado.razaoSocialEmpresa && (
                      ...
                      <p className="text-xs text-slate-600 mt-1">
                        CNPJ: {pedidoSelecionado?.cnpjEmpresa || '—'}
                      </p>

                      <p className="text-xs text-slate-600 mt-1">
                        {pedidoSelecionado?.logradouroEmpresa || '—'}
                        {pedidoSelecionado?.numeroEmpresa
                          ? `, Nº ${pedidoSelecionado.numeroEmpresa}`
                          : ''}
                        {pedidoSelecionado?.bairroEmpresa
                          ? ` - ${pedidoSelecionado.bairroEmpresa}`
                          : ''}
                      </p>

                      <p className="text-xs text-slate-600">
                        {pedidoSelecionado?.cepEmpresa || '—'} -{' '}
                        {pedidoSelecionado?.cidadeEmpresa || '—'},{' '}
                        {pedidoSelecionado?.ufEmpresa || '—'}
                      </p>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xl font-black text-slate-800 mt-1">
                        Pedido {pedidoSelecionado?.codigoPedidoFormatado || '—'}
                      </p>

                      <p className="text-xs font-semibold text-slate-600 mt-1">
                        Data:{' '}
                        {pedidoSelecionado
                          ? formatDate(pedidoSelecionado.dataCriacao)
                          : '—'}
                      </p>
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-300">
                    ...
                    <p className="text-sm font-black text-slate-800">
                      {pedidoSelecionado?.nomeCliente || 'Consumidor Final'}
                    </p>

                    {pedidoSelecionado?.nomeFantasiaCliente &&
                      pedidoSelecionado.nomeFantasiaCliente !== pedidoSelecionado.nomeCliente && (
                        <p className="text-xs font-semibold text-slate-600">
                          {pedidoSelecionado.nomeFantasiaCliente}
                        </p>
                      )}
                    ...
                    <p className="text-xs text-slate-600 mt-1">
                      CNPJ/CPF: {pedidoSelecionado?.documentoCliente || '—'}
                    </p>

                    <p className="text-xs text-slate-600 mt-1">
                      {pedidoSelecionado?.logradouroCliente || '—'}
                      {pedidoSelecionado?.numeroCliente
                        ? `, Nº ${pedidoSelecionado.numeroCliente}`
                        : ''}
                      {pedidoSelecionado?.bairroCliente
                        ? ` - ${pedidoSelecionado.bairroCliente}`
                        : ''}
                    </p>

                    <p className="text-xs text-slate-600">
                      {pedidoSelecionado?.cepCliente || '—'} -{' '}
                      {pedidoSelecionado?.cidadeCliente || '—'},{' '}
                      {pedidoSelecionado?.ufCliente || '—'}
                    </p>
                ...
                <div className="print:hidden">
                  ...
                  <h2 className="text-2xl font-black text-slate-800 flex items-center gap-2">
                    ...
                    {loadingTriagem
                      ? '...'
                      : (
                        pedidoSelecionado?.codigoPedidoFormatado ||
                        '—'
                      )}
              ...
              <button
                ...
                className="p-2 text-slate-400 hover:bg-slate-200 rounded-full cursor-pointer transition-colors print:hidden"
              >
                <X size={20} />
              </button>
            ...
            <div className="p-6 bg-white overflow-y-auto flex-1 print:overflow-visible">
                    ...
                    <table className="w-full text-left border-collapse print:border print:border-slate-200">
                      ...
                      <thead>
                        <tr className="border-b border-slate-200 text-[11px] uppercase tracking-wider text-slate-400 font-black print:bg-slate-100 print:text-slate-600">
                          ...
                          'comercial' ? (
                            <>
                              <th className="pb-3 print:p-3">
                                Produto
                              </th>

                              <th className="pb-3 print:p-3 text-right">
                                Preço Unit.
                              </th>
                            ...
                            <>
                              ...
                              <th className="pb-3 print:p-3 text-center">
                                Unidades
                              </th>
                        ...
                        {itensFiltrados.map(
                          (
                            ...
                            <tr
                              ...
                              className="hover:bg-slate-50/50 print:border-b print:border-slate-100"
                            >

                              {activeTabTriagem ===
                              'comercial' ? (
                                <>
                                  <td className="py-3 font-bold text-slate-700 print:p-3">
                                    ...
                                    <span className="hidden print:block text-[10px] text-slate-400 font-medium mt-0.5">
                                      Código:{' '}
                                  ...
                                  <td className="py-3 text-center font-black text-slate-800 print:p-3 bg-slate-50 print:bg-transparent">
                                    {
                                      item.quantidade
                                    }
                                  </td>

                                  <td className="py-3 text-right text-slate-500 font-semibold print:p-3">
                                    {formatCurrency(
                                      item.precoUnitario
                                    )}
                                  </td>

                                  <td className="py-3 text-right font-black text-slate-800 print:p-3">
                                    {formatCurrency(
                                      item.quantidade *
                                      item.precoUnitario
                                    )}
              ...
              {!loadingTriagem &&
                activeTabTriagem ===
                  'comercial' &&
                pedidoSelecionado && (
                  <div className="mt-6 pt-5 border-t border-slate-200 print:mt-6 print:pt-4">

                    <div className="grid grid-cols-3 gap-4 print:grid-cols-3">

                      <div className="border border-slate-200 rounded-lg p-3 print:border-slate-300">
                      ...
                      <div className="border border-slate-200 rounded-lg p-3 print:border-slate-300">
                        ...
                        <p className="text-lg font-black text-slate-800 mt-1">
                          {
                            pedidoSelecionado.somaQuantidades
                          }
                        </p>
                      </div>
                      ...
                      <div className="border border-slate-200 rounded-lg p-3 print:border-slate-300">
                        ...
                        <p className="text-lg font-black text-slate-800 mt-1">
                          {formatCurrency(
                            pedidoSelecionado.totalProdutos
                          )}
                        </p>
                      </div>
                    ...
                    <div className="mt-4 flex justify-end">
                      ...
                      <div className="text-right bg-slate-50 px-8 py-4 rounded-xl border border-slate-200 print:bg-white print:border-slate-300 print:px-6 print:py-3">
                        ...
                        <p className="text-3xl font-black text-amber-500">
                          {formatCurrency(
                            pedidoSelecionado.valorTotalPedido
                          )}
                        </p>
                    ...
                    <div className="mt-6 print:mt-5">
                      ...
                      <div className="border border-slate-200 rounded-lg p-3 min-h-[50px] print:border-slate-300">

                        <p className="text-xs text-slate-600 whitespace-pre-wrap">
                          {
                            pedidoSelecionado.observacaoNegociacao ||
                    ...
                    <div>
                      <p className="font-bold text-emerald-800">
                        {clienteSelecionado.nome_fantasia ||
                          clienteSelecionado.nome_ou_razao_social}
                      </p>
                ...
                <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {itensManuais
                      .filter(
                        i =>
                          i.eh_fabricacao_propria
                      )
                      .map(
                        (item, idx) => (
                          <div
                            key={idx}
                            className="flex justify-between items-center bg-white p-3 rounded-xl border border-emerald-100 shadow-sm"
                          >

                            <div className="flex-1 min-w-0">

                              <p className="font-bold text-slate-700 text-sm break-words pr-2">
                                {item.nome}
                              </p>
                    ...
                    {itensManuais.filter(
                      ...
                      <p className="text-xs text-emerald-600/60 font-medium italic text-center py-4">
                ...
                <div className="bg-purple-50/50 border border-purple-100 rounded-2xl p-4">
                  ...
                  <div className="space-y-2">
                    ...
                    {itensManuais
                      ...
                      .map(
                        (item, idx) => (
                          <div
                            key={idx}
                            className="flex justify-between items-center bg-white p-3 rounded-xl border border-purple-100 shadow-sm"
                          >
                    ...
                    {itensManuais.filter(
                      ...
                      <p className="text-xs text-purple-600/60 font-medium italic text-center py-4">
                        Nenhum item adicionado.
                      </p>
                    )}
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden border border-slate-200">
            ...
            <div className="p-6 text-center space-y-4">
              ...
              <p className="text-slate-500 font-medium text-sm">
                ...
                e enviá-lo para a fila de produção (Fábrica)?
              </p>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm print:hidden">
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden border border-slate-200">
            ...
            <div className="p-6 text-center space-y-4">
              ...
              <p className="text-slate-500 font-medium text-sm">
                Tem certeza que deseja marcar o pedido{' '}
                <strong className="text-slate-700">
                  {
                    pedidoSelecionado?.codigoPedidoFormatado
                  }
