...
public class InteligenciaReadRepository : IInteligenciaReadRepository
{
    ...
    private static readonly int StatusCancelado           = (int)StatusFilaLogistica.Cancelado;
    private static readonly int StatusClienteAprovado     = (int)StatusCadastroCliente.Aprovado;
    private static readonly int SegmentoB2B               = (int)TipoSegmentoCliente.B2B;
    ...
    // =========================================================================
    // MÉTODO 1 — Dashboard Principal (QueryMultiple = 5 queries em 1 roundtrip)
    // =========================================================================
    public async Task<DashboardPrincipalResponse> ObterDashboardPrincipalAsync(Guid tenantId)
    {
        const string sql = @"
            -- Q1: Cards (receita validada, validação pendente, carteira ativa, risco evasão)
            SELECT
                COALESCE(SUM(p.valor_total_pedido) FILTER (WHERE p.status_logistica <> @Cancelado), 0)   AS ReceitaValidada,
                COUNT(p.id) FILTER (WHERE p.status_logistica = @AguardandoValidacao)                      AS ValidacaoPendenteQtd,
                (SELECT COUNT(c.id)
                 FROM clientes c
                 WHERE c.empresa_id = @TenantId
                   AND c.status_cadastro = @StatusAprovado
                   AND c.tipo_segmento   = @SegmentoB2B)                                                  AS CarteiraAtivaQtd,
                (SELECT COUNT(DISTINCT c2.id)
                 FROM clientes c2
                 WHERE c2.empresa_id    = @TenantId
                   AND c2.status_cadastro = @StatusAprovado
                   AND c2.tipo_segmento   = @SegmentoB2B
                   AND NOT EXISTS (
                     SELECT 1 FROM pedidos p2
                     WHERE p2.cliente_id  = c2.id
                       AND p2.empresa_id  = @TenantId
                       AND p2.data_criacao >= NOW() - INTERVAL '60 days'
                   ))                                                                                      AS RiscoEvasaoQtd
            FROM pedidos p
            ...
            WHERE ped.empresa_id = @TenantId
              AND ped.status_logistica <> @Cancelado;

            -- Q3: Top 5 mais vendidos (em quantidade) - Produtos Próprios da Fábrica
            SELECT
            ...
            WHERE ped3.empresa_id = @TenantId
              AND ped3.status_logistica <> @Cancelado
              AND pi3.eh_fabricacao_propria_snapshot = TRUE
            GROUP BY p3.descricao
            ORDER BY QtdVendida DESC
            LIMIT 5;

            -- Q4: Top 5 mais vendidos (em quantidade) - Produtos de Terceiros (Revenda)
            SELECT
            ...
            WHERE ped4.empresa_id = @TenantId
              AND ped4.status_logistica <> @Cancelado
